...
//	private static final Config config = ConfigFactory.parseResources("config.conf");
	...
	public Response restAssured(String body, Map excelKeysAndValues, org.slf4j.Logger logger) throws IOException {
		...
		try {
			...
			if (xlsEnvironment.toString().equals("_QA")) {
				...
				RestAssured.config = RestAssured.config()
			        ...
			        keyStore.load( new FileInputStream(KeyStoreLocation.P12FILE),KeyStoreLocation.P12PASSWORD.toCharArray());

			        org.apache.http.conn.ssl.SSLSocketFactory clientAuthFactory = new org.apache.http.conn.ssl.SSLSocketFactory(keyStore, KeyStoreLocation.P12PASSWORD);
			...
			logger.debug("\n\n\n\t\t #######CAUGHT EXCEPTION IN EXECUTION METHOD UTIL FILE - LINE 381 ####### :\n\n\n\t\t\t\t\t\t\t" + e.toString()
					+ "\n");
