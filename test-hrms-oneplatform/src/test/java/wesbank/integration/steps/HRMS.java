...
public class HRMS extends Scenario {
	public int excelCount;

	@Given("HRMS_Services \\( {string} ) service is up and running")
	 public void that_hrms_services_service_is_up_and_running(String serviceName) throws Exception {

        excelCount = apiUtils.readExcelReturnCount(HRMS.getString("root.serviceName." + serviceName + ".testData"), HRMS.getString("root.serviceName." + serviceName + ".sheetName"), logger);
        ...
        for (int testID = 0; excelCount != testID; ) {
            if (excelCount == testID) {
                ...
                while (excelCount != testID) {
                    testID++;
                    i_have_testdata_in_excel(HRMS.getString("root.serviceName." + serviceName + ".testData"), HRMS.getString("root.serviceName." + serviceName + ".sheetName"), testID, logger);
    ...
    public void i_have_testdata_in_excel(String destFile, String serviceName, int testID, Logger logger) throws Exception {
        ...
        if (!apiUtils.isBDDComplient(apiDto.excelRequestMap, logger)) {
            logger.error("\t\t ####### EXITING, kindly update (Given When Then) syntax in BDDSyntax field for below scenario####### \n\n\t\t\t\t\t\t\t\t " + serviceName + " : " + apiDto.excelRequestMap.get("scenario") + "\n");
            throw new RuntimeException("Exiting Scenario, kindly update (Given When Then) syntax in BDDSyntax field. :) ");
        }
    ...
    private void i_map_the_raw_test_data_to_a_XML_request() throws Exception {
        body = null;
        if (!HRMS.getString("root.serviceName." + this.serviceName + ".reqType").equals("template")) {
            body = apiUtils.MapToXML(apiDto.excelRequestMap).toString().replace("$", "").replace("[]", "");
        } else {
            body = API_Utility_Functions.parseXML(HRMS.getString("root.serviceName." + this.serviceName + ".filename"), apiDto.excelRequestMap);
        }
    }
